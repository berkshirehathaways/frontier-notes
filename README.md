...
프론티어노트의 디자인은 두 문서를 기준으로 관리합니다.

- `DESIGN.md` — 색, 타이포그래피, 간격, 컴포넌트 원칙
- `docs/design/content-hierarchy.md` — 이슈, 노트, 피플 등 콘텐츠 타입별 화면 위계와 포맷

UI를 수정할 때는 먼저 어떤 콘텐츠 타입의 위계를 바꾸는지 확인하고, 반복되는 패턴이면
두 문서 중 하나에 규칙을 남깁니다. 공개 UI에는 `i-am-human`, `standalone` 같은 내부 slug를
